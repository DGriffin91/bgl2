uniform sampler2D B_shadow_texture;

uniform mat4 B_shadow_clip_from_world;
uniform vec3 B_directional_light_dir;
uniform vec3 B_directional_light_color;

uniform samplerCube B_specular_map;
uniform samplerCube B_diffuse_map;
uniform float B_env_intensity;

uniform int B_light_count;
uniform vec4 B_point_light_position_range[MAX_POINT_LIGHTS];
...
vec3 apply_pbr_lighting(vec3 V, vec3 diffuse_color, vec3 F0, vec3 vert_normal, vec3 normal, float perceptual_roughness,
    ...
    vec4 shadow_clip = B_shadow_clip_from_world * vec4(ws_position + vert_normal * normal_bias, 1.0);
    ...
    output_color += directional_light(V, F0, diffuse_color, normal, roughness, diffuse_transmission, dir_shadow, B_directional_light_dir, B_directional_light_color);
...

    {
        ...
        vec3 env_diffuse = rgbe2rgb(textureCubeLod(B_diffuse_map, vec3(normal.xy, -normal.z), 0.0)) * B_env_intensity;

        vec3 env_specular = vec3(0.0);

        vec3 dir = reflect(-V, normal);
        env_specular = rgbe2rgb(textureCubeLod(B_specular_map, vec3(dir.xy, -dir.z), perceptual_roughness * mip_levels)) * B_env_intensity;
    ...
    for (int i = 0; i < MAX_POINT_LIGHTS; i++) {
        if (i < B_light_count) {
            vec4 light_position_range = B_point_light_position_range[i];
